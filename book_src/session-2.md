...
| ------------------ | ---------------------------------------------------------- | --------------------------------- |
| s                  | $\{a[i] \, \vert \, i \in [0, n) \}$                       | s は "abcde"                      |
| s[index]           | $\{a[index] \}$                                            | s[0] は "a"                       |
| s[start:]          | $\{a[i] \, \vert \, i \in [start, n) \}$                   | s[1:] は "bcde"                   |
| s[:stop]           | $\{a[i] \, \vert \, i \in [0, stop) \}$                    | s[:4] は "abcd"                   |
| s[start:stop]      | $\{a[i] \, \vert \, i \in [start, stop) \}$                | s[1:4] は "bcd"                   |
| s[start:stop:step] | $\{a[i] \, \vert \, i \in [start, stop) \}$ の step 個飛び | s[::2] (つまり s[0:5:2]) は "ace" |
